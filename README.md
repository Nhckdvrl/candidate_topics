...
|---:|---|---|---|
| 1 | [28 — Information Non-Monotonicity under Progressive Truthful Clues](./28_progressive_truthful_clue_reversal/) | **ACTIVE / G0 PASSED / STRUCTURE ANALYZED** | `GO_REVERSAL_OBJECT`: `8,102/120,353` adjacent correct-state transitions reverse (`6.7319%`), across 760 questions and 93 AI configs; 3,871 are strict-alias-supported. Frozen analysis finds specificity capture, a direct lexical-competitor subset, and mixed transient/persistent recovery. Next: preregister a fixed-clue-multiset order intervention; no mechanism work yet. See [`G0_RESULTS.md`](./28_progressive_truthful_clue_reversal/G0_RESULTS.md) and [`ANALYSIS_RESULTS.md`](./28_progressive_truthful_clue_reversal/ANALYSIS_RESULTS.md). |
| 2 | [24 — Where Does Closed-Loop Robustness Live in Hierarchical Robot Foundation Policies?](./24_hierarchical_feedback_attribution/) | **ACTIVE / G0+G1+G2 RUN** | G0 (`630/630`, `WBC_LEVEL_DOMINATES`): WBC seam accounts for essentially all measured recovery. G1 (100N, `NAVIGATION_CHANNEL_CAUSES_REVERSAL`): the sign flip lives in the navigation/base channel (`N_100,left=+0.20 [0.067,0.367]`, `N_100,right=-0.20 [-0.367,-0.033]`); post-result audit tightened two overclaims. G2 (50N/150N added, `REVERSAL_CONFIRMED_AT_SOME_FORCES_ONLY`): only `100N` clears the frozen bar; `left`'s effect decays monotonically to exactly `0.000` (CI `[0,0]`) by `150N`, `right`'s peaks at `100N` rather than trending. See [`G0_RESULTS.md`](./24_hierarchical_feedback_attribution/G0_RESULTS.md), [`G1_RESULTS.md`](./24_hierarchical_feedback_attribution/G1_RESULTS.md), [`G2_RESULTS.md`](./24_hierarchical_feedback_attribution/G2_RESULTS.md). |
| 3 | [26 — Temporal Scope Interference & Reinstatement](./26_temporal_scope_interference_reinstatement/) | **REGISTERED / G0 IMPLEMENTED / PREFLIGHT NEXT** | Run the frozen exact-eligibility preflight on ChronoScope; only if >=512 matched targets exist, execute the fixed neutral-distance / same-entity / bounded-present / weak-reinstatement G0 on Qwen2.5-7B-Instruct. |
| 4 | [25 — When Does Test-Time Reasoning Help Context Use, and When Does It Hurt?](./25_reasoning_context_use_boundary/) | **REGISTERED / REPRODUCTION RECEIPT NEXT** | Reproduce the pinned Weakest-Link Qwen3-8B think/no-think seed relation; only then run the frozen matched atomic-vs-composed G0. |
| 5 | [16 — Does Citation Turn Hypotheses into Facts?](./16_citation_transmutation/) | **REGISTERED / G0 NOT RUN** | Identification is hardened; next step is the high-precision evidence-provenance G0. |
