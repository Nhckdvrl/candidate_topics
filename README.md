...
| 2 | [28 — Information Non-Monotonicity under Progressive Truthful Clues](./28_progressive_truthful_clue_reversal/) | **G0 OBJECT ESTABLISHED / G1 ORDER BRANCH STOPPED** | G0 remains strong (`8,102/120,353`, 6.7319%), but the frozen outcome-blind adjacent-swap G1 returned `STOP_ORDER_DEPENDENCE`: 15 original and 15 swap reversals, `delta_order=0.0000`, and same-multiset final-error difference `-0.01606` with CI crossing zero. The path-dependence paper upgrade is not supported; do not tune model/prompt/panel/scorer or enter mechanism. See [`G1_RESULTS.md`](./28_progressive_truthful_clue_reversal/G1_RESULTS.md). |
