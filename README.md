...
| 2 | [26 — Temporal Scope Interference & Reinstatement](./26_temporal_scope_interference_reinstatement/) | **REGISTERED / G0 IMPLEMENTED / PREFLIGHT NEXT** | Run the frozen exact-eligibility preflight on ChronoScope; only if >=512 matched targets exist, execute the fixed neutral-distance / same-entity / bounded-present / weak-reinstatement G0 on Qwen2.5-7B-Instruct. |
| 3 | [25 — When Does Test-Time Reasoning Help Context Use, and When Does It Hurt?](./25_reasoning_context_use_boundary/) | **REGISTERED / REPRODUCTION RECEIPT NEXT** | Reproduce the pinned Weakest-Link Qwen3-8B think/no-think seed relation; only then run the frozen matched atomic-vs-composed G0. |
| 4 | [16 — Does Citation Turn Hypotheses into Facts?](./16_citation_transmutation/) | **REGISTERED / G0 NOT RUN** | Identification is hardened; next step is the high-precision evidence-provenance G0. |
...
|---|---|---|---|
| [28 — Information Non-Monotonicity under Progressive Truthful Clues](./28_progressive_truthful_clue_reversal/) | **ARCHIVED / G0 PHENOMENON REAL / CONTROLLED EXPLANATIONS FAILED** | G0 found 8,102 released reversals, but G1 found no adjacent-order effect and G2a found destructive conjunction in only `1/229` jointly sufficient cases (`0.44%`); the specificity-capture/composition paper route lacks controlled support. | [Archive summary](./28_progressive_truthful_clue_reversal/ARCHIVE_SUMMARY.md) |
