...
| --- | --- | --- | --- |
| 1 | [19 — Do Robot Foundation Policies Learn Task-Structured Feedback?](./19_task_structured_feedback/) | Given equally large body perturbations, does a foundation robot policy selectively correct deviations that change active end-effector task geometry while accommodating Jacobian-null, task-equivalent body deviations? | On released Ψ₀ + SIMPLE CloseDoor, physically construct equal-norm wrist-moving vs full-wrist-pose-null right-arm perturbations at the same deployed state, re-render both observations, use common-random-number first-target queries, and require episode-level `ΔR = R_task-R_null >= 0.20` with 95% CI above zero. |
