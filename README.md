...
|---:|---|---|---|
| 1 | [24 — Where Does Closed-Loop Robustness Live in Hierarchical Robot Foundation Policies?](./24_hierarchical_feedback_attribution/) | **ACTIVE / G0+G1 RUN** | G0 (`630/630`, `WBC_LEVEL_DOMINATES`): the WBC seam accounts for essentially all measured recovery (`delta_low = 0.300`, CI `[0.244, 0.356]`); pooled VLA-level contribution not distinguishable from zero (`delta_high = 0.017`, CI `[-0.050, 0.083]`) — but that pooled near-zero hides a left/right sign flip. G1 (`240` rows, `NAVIGATION_CHANNEL_CAUSES_REVERSAL`): factoring the VLA command shows the flip lives entirely in the navigation/base channel (`+0.20/+0.23` left, `-0.20` right, all CI-excluding-zero); the upper-body channel's effect is at or below the minimum worthy effect in all four estimates. See [`G0_RESULTS.md`](./24_hierarchical_feedback_attribution/G0_RESULTS.md) and [`G1_RESULTS.md`](./24_hierarchical_feedback_attribution/G1_RESULTS.md). |
