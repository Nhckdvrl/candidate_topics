...
Very recent DLM work has already shown that local token confidence can diverge from global reasoning correctness and that exposed confidence can diverge from internal representations. Therefore this project must not ask the broad question "is confidence equal to correctness?"
...
The key comparison is **self-consistency vs world correctness**.
...
For each base problem, construct matched A/B/C/D trajectories of the same overall format and approximately the same token budget. Score them with the exact frozen confidence protocol used by the target DLM.
...
That changes what the model's confidence can be interpreted as measuring before any downstream method is proposed.
