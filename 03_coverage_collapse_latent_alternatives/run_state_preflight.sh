...
pids=()
for i in "${!TAGS[@]}"; do
  TAG=${TAGS[$i]}; CKPT="$RUN/checkpoint-${CKPTS[$TAG]}"
  [[ -d "$CKPT" ]] || { echo "Missing $CKPT" >&2; exit 1; }
  GPU=${GPU_IDS[$((i % ${#GPU_IDS[@]}))]}
  CUDA_VISIBLE_DEVICES="$GPU" python src/extract_branch_states.py \
    --forks artifacts/forks.jsonl --model "$CKPT" --tag "$TAG" --limit "$LIMIT" --output-dir "$OUT" &
  pids+=("$!")
done
for pid in "${pids[@]}"; do wait "$pid"; done
python src/summarize_state_preflight.py --input-dir "$OUT" --tags e01,e02,e04,e16 --output artifacts/state_preflight.csv
