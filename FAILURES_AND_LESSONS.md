...
15. **Order the search from the verified seam, not from the concept.** Topics 09, 19 and 23 each proposed an attractive second-order concept first and only later discovered whether the experimental object existed. Prefer starting from a causal seam or anomaly already observed in the released code or behaviour, then asking the scientific question one step away from it.
16. **Prove the instrument before the hypothesis, and prove the right thing about it.** An intervention that reproduces the original system perfectly under undisturbed conditions has *not* been shown to cut anything causal: if nothing drives the state off the recorded trajectory, no feedback needs to act, and a purely feedforward stage would pass the same fidelity check. Seam liveness — same command, same internal state, same clock, only the observation changed — is a separate gate and must pass before the causal quantity is defined. (Topic 24 P0 vs P0b.)
17. **Name a difference by what it actually cuts.** `high-level replay minus low-level replay` is not "the low-level controller contribution" when servo, actuator dynamics, mechanics and task tolerance all survive the cut. Fix the naming before the number exists, and check which output channels the cut layer can even influence — in Topic 24 the arms and hands below the VLA seam turned out to be open-loop, bounding that term to locomotion feedback.
18. **Wall-clock coupling is a measurement confound, not a detail.** A controller that interpolates against `time.monotonic()` makes model-inference latency an input, so benchmark outcomes can depend on machine load rather than on the policy. Declare a nominal control clock, record it in every row, and report real-clock sensitivity separately from hypothesis evidence.
19. **A strong observational gradient is not yet an aggregation law.** Topic 28 established 8,102 released truthful-clue reversals and a monotonic specificity gradient, but adjacent order had exactly zero paired effect and destructive conjunction appeared in only `1/229` jointly sufficient cases. Preserve the phenomenon, but archive the explanatory route when the matched intervention and strongest compositional cell both fail; do not promote the correlate through natural-substitution, model, scorer, or mechanism search.
20. **Cross-model clustering is not yet a population of transferable semantic traps.** Topic 28's final artifact-only preflight found cross-family co-reversal overlap `1.765x` above an agent-marginal-preserving null, but only 17 boundaries/questions met the full same-exact-wrong, cross-family trap definition versus frozen 20/20 gates. Enforce family independence on both the flip and the shared answer: repeated configs from one family cannot establish a transferable competitor. When the corrected support gate fails, do not launch modern-model confirmation.
