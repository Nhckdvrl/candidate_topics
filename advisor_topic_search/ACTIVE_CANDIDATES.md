...
## B1. Progressive Truthful-Clue Reversal

旧问题：

> Quiz Bowl 中增加一条 truthful clue 后，模型是否会从 correct -> wrong？

**New audit:** 当前 formulation 太依赖 Quiz Bowl trajectory，本质上仍容易成为 benchmark-specific model behavior。

只有能提升为更一般的：

> **information monotonicity / evidence integration under accumulating truthful evidence**

并且在至少两个不同 task family / 多模型上仍然成立，才有资格重新晋级。

released ~304k response rows 仍然非常适合做一个廉价 prevalence screen，但 screen 的正结果本身**不等于论文题成立**。

**Status:** `DEMOTED / CHEAP_SCREEN_ONLY / REQUIRE_CROSS_TASK_CONSTRUCT`.

---

## B2. Positional Imprinting of Parametric Knowledge
