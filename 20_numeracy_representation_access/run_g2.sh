...
# Frozen Topic 20 G2 runner.
# Generates seed-0 train/val and untouched seed-20260825 int-sci data from the
# pinned upstream revision, preserves the fresh causal test set, then runs the
# preregistered notation-competition intervention.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
UPSTREAM_ROOT="${UPSTREAM_ROOT:-${1:-}}"
MODEL="${MODEL:-Qwen/Qwen3-8B}"
PYTHON_BIN="${PYTHON_BIN:-python}"
OUT_DIR="${OUT_DIR:-${ROOT}/20_numeracy_representation_access/artifacts/g2}"
BATCH_SIZE="${BATCH_SIZE:-8}"
PINNED_UPSTREAM="9e1be04b69965662886c79d543936389c5407d27"
FRESH_SEED="20260825"

if [[ -z "${UPSTREAM_ROOT}" ]]; then
  echo "Usage: UPSTREAM_ROOT=/path/to/Numeracy-Probing bash 20_numeracy_representation_access/run_g2.sh"
  echo "   or: bash 20_numeracy_representation_access/run_g2.sh /path/to/Numeracy-Probing"
  exit 2
fi
...
mkdir -p "${OUT_DIR}"
TMP="$(mktemp -d -t topic20_g2.XXXXXX)"
trap 'rm -rf "${TMP}"' EXIT
...
"${PYTHON_BIN}" "${ROOT}/20_numeracy_representation_access/g2_notation_competition.py" \
